...
@dataclass
class TorchmetricsConfig(io.IOMixin):

    class_path: str
    task: Literal["lm", "classification", "regression", "multitask"]
    metric_name: str
    ...
    def get_instance(self) -> torchmetrics.Metric:
        ...
        # Check if this is a bionemo custom metric
        if self.class_path.startswith("bionemo."):
            module_path, class_name = self.class_path.rsplit(".", 1)
            module = importlib.import_module(module_path)
            cls_ = getattr(module, class_name)
            return cls_(**kwargs)
        
        # Otherwise, assume it's a torchmetrics metric
